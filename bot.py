...
BOT_TEXTS = {
    'pl': {
        ...
        'balance_text': 'Twój balans: {c} żetonów', 'lang_ok': '✅ Język zmieniony!', 'token': 'żetonów', 'buy_m': '💳 Wybierz pakiet:',
    ...
    'ua': {
        ...
        'balance_text': 'Ваш баланс: {c} жетонів', 'lang_ok': '✅ Мову змінено!', 'token': 'жетонів', 'buy_m': '💳 Оберіть пакет:',
        'ref_t': '🔗 Посилання:\n<code>https://t.me/{b}?start=ref{u}</code>'
    },
    'ru': {
        'welcome': 'Добро пожаловать! 🎰', 'play': '🎰 Играть сейчас', 'buy': '💳 Купить жетоны', 'set': '⚙️ Язык', 'bal': '💰 Мой баланс', 'ref': '👥 Рефералы',
        'balance_text': 'Ваш баланс: {c} жетонов', 'lang_ok': '✅ Язык изменен!', 'token': 'жетонов', 'buy_m': '💳 Выберите пакет:',
        'ref_t': '🔗 Ссылка:\n<code>https://t.me/{b}?start=ref{u}</code>'
    },
    'en': {
        'welcome': 'Welcome! 🎰', 'play': '🎰 Play Now', 'buy': '💳 Buy Coins', 'set': '⚙️ Language', 'bal': '💰 My Balance', 'ref': '👥 Referrals',
        'balance_text': 'Your balance: {c} coins', 'lang_ok': '✅ Language changed!', 'token': 'coins', 'buy_m': '💳 Choose package:',
        'ref_t': '🔗 Link:\n<code>https://t.me/{b}?start=ref{u}</code>'
    }
}
...
async def handle_buttons(message: Message):
    ...
    if any(txt == BOT_TEXTS[l]['buy'] for l in BOT_TEXTS):
        ...
        await message.answer(BOT_TEXTS[lang]['ref_t'].format(b=bot_info.username, u=uid), parse_mode="HTML")
...
async def api_get_balance(request):
    ...
    try:
        ...
        if not uid: return web.json_response({"ok": False}, headers=headers)
        _, coins, _ = get_user_data(uid)
        print(f"💰 API: Sending balance {coins} to user {uid}")
        return web.json_response({"ok": True, "balance": int(coins)}, headers=headers)
