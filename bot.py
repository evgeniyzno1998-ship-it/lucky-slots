...
# Railway автоматически дает домен, используем его
...
BOT_TEXTS = {
    'pl': {
        ...
        'balance_text': 'Twój balans: {c} żetonów', 'lang_ok': '✅ Język zmieniony!', 'token': 'żetonów',
        'ref_text': '🔗 Twój link (kliknij aby skopiować):\n<code>https://t.me/{b}?start=ref{u}</code>\n\n👥 Poleceni: {cnt}'
    },
    'ua': {
        'welcome': 'Вітаємо у Lucky Slots! 🎰', 'play': '🎰 Грати зараз', 'buy': '💳 Купити жетони', 'set': '⚙️ Мова', 'bal': '💰 Мій баланс', 'ref': '👥 Запросити друга',
        'balance_text': 'Ваш баланс: {c} жетонів', 'lang_ok': '✅ Мову змінено!', 'token': 'жетонів',
    ...
    'ru': {
        ...
        'balance_text': 'Ваш баланс: {c} жетонов', 'lang_ok': '✅ Язык изменен!', 'token': 'жетонов',
    ...
    'en': {
        ...
        'balance_text': 'Your balance: {c} coins', 'lang_ok': '✅ Language changed!', 'token': 'coins',
...
def init_db():
    with sqlite3.connect('users.db') as conn:
        conn.execute('''CREATE TABLE IF NOT EXISTS users (
            ...
            referrals_count INTEGER DEFAULT 0, coins INTEGER DEFAULT 0, language TEXT DEFAULT 'pl')''')
...
def packages_keyboard(lang):
    ...
    for l, p in {"50": 0.50, "100": 0.90, "500": 4.00}.items():
        builder.button(text=f"{l} {t_name} — {p} USDT", callback_data=f"buy_{l}")
...
async def cmd_start(message: Message):
    ...
    # Если нажали "Депозит" в игре
    if len(args) > 1 and args[1] == "deposit":
        await message.answer(BOT_TEXTS[lang]['buy'], reply_markup=packages_keyboard(lang))
        return

    await message.answer(BOT_TEXTS[lang]['welcome'], reply_markup=main_menu(user_id, bot_info.username))

# Обработка всех кнопок через поиск совпадений в словаре
@dp.message(F.text)
async def handle_buttons(message: Message):
    ...
    if any(txt == BOT_TEXTS[l]['buy'] for l in BOT_TEXTS):
        ...
        await message.answer(BOT_TEXTS[lang]['ref_text'].format(b=bot_info.username, u=uid, cnt=refs), parse_mode="HTML")
...
async def set_lang(call: CallbackQuery):
    ...
    await call.message.answer(BOT_TEXTS[lang_code]['welcome'], reply_markup=main_menu(call.from_user.id, bot_info.username))
